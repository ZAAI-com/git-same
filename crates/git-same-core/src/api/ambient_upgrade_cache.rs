...
//! repo, the extension sends `REFRESH /path` over the socket. The monitor then
...
//!
//! The cache is not persisted to disk: it lives only for the current monitor
//! run. Restarting the monitor returns all ambient repos to gray until the user
