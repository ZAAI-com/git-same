...
//!
//! All scanning logic lives in `git_same_core::api::RepoScanService`. This module
//! is just the CLI surface (start/stop/status) plus the daemon loop and
//! socket handler that drive the service.

use crate::cli::DaemonArgs;
use git_same_core::api::{AmbientUpgradeCache, OwnerTypeCache, RepoScanService};
...
pub async fn run(args: &DaemonArgs, config: &Config, output: &Output) -> Result<()> {
    ...
    #[cfg(unix)]
    let socket_listener = git_same_core::ipc::UnixSocketListener::new(ipc_config.socket_path());
...
fn spawn_owner_classifier(config: Config, cache: OwnerTypeCache) {
    tokio::spawn(async move {
        ...
        let token = match git_same_core::auth::gh_cli::get_token() {
            Ok(t) => t,
            Err(e) => {
        ...
        let ws_provider = git_same_core::config::WorkspaceProvider::default();
        let provider = match git_same_core::provider::create_provider(&ws_provider, &token) {
            Ok(p) => p,
            Err(e) => {
...
fn collect_owner_names(config: &Config) -> Vec<String> {
    ...
    for ws_path in &config.workspaces {
        ...
        let ws_config = match git_same_core::config::WorkspaceStore::load(&root) {
            Ok(ws) => ws,
            Err(_) => continue,
        };
