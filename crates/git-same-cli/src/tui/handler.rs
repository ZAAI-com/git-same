...
use git_same_core::cache::SyncHistoryManager;
use git_same_core::config::WorkspaceManager;
use git_same_core::domain::RepoPathTemplate;
use git_same_core::types::SyncHistoryEntry;
    ...
    match msg {
        ...
        BackendMessage::SetupCheckResults(entries) => {
            ...
            if let Some(ref mut setup) = app.setup_state {
                ...
                let results = entries
                    .iter()
                    .map(|e| git_same_core::checks::CheckResult {
                        name: e.name.clone(),
                        passed: e.passed,
