...
use git_same_core::config::{Config, SyncMode, WorkspaceConfig, WorkspaceManager};
...
fn render_workspace_detail(app: &App, ws: &WorkspaceConfig, frame: &mut Frame, area: Rect) {
    ...
    let ws_tilde_path = git_same_core::config::workspace::tilde_collapse_path(&ws.root_path);
